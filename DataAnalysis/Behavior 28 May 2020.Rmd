...
# Create a color ramp
cbbPalette <- c("black", "deepskyblue3", "goldenrod3")
...
mod <- lme(Total ~  log10Bd * (Treatment-1) + Exp_Day, random = ~1|ID_Number, data = dat)
...
# Plot data
ggplot(data = mat2, aes(y = Weight_g, x = log10(Bd+1), col = Treatment))+ 
...
# Used generalized linear mixed effect model with log10(Bd+1) load as the response variable and 
mod <- lme(Weight_g ~  log10Bd * (Treatment-1) + Exp_Day.y, random = ~1|ID_Number, data = mat2)
